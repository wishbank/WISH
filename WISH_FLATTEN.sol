...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function transferFrom(
        ...
        _spendAllowance(from, spender, wishes);
        _transfer(from, to, wishes);
        return true;
    }
    ...
    function _transfer(
        ...
        _beforeTokenTransfer(from, to, wishes);
        ...
        unchecked {
            _balances[from] = fromBalance - wishes;
        }
        _balances[to] += wishes;

        emit Transfer(from, to, wishes);

        _afterTokenTransfer(from, to, wishes);
    }

    function _mint(address account, uint256 wishes) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _beforeTokenTransfer(address(0), account, wishes);

        _totalSupply += wishes;
        _balances[account] += wishes;
        emit Transfer(address(0), account, wishes);

        _afterTokenTransfer(address(0), account, wishes);
    }

    function _burn(address account, uint256 wishes) internal virtual {
        require(account != address(0), "ERC20: burn from the zero address");

        _beforeTokenTransfer(account, address(0), wishes);

        uint256 accountBalance = _balances[account];
        require(accountBalance >= wishes, "ERC20: burn amount exceeds balance");
        unchecked {
            _balances[account] = accountBalance - wishes;
        }
        _totalSupply -= wishes;

        emit Transfer(account, address(0), wishes);

        _afterTokenTransfer(account, address(0), wishes);
    }
...
contract WISH is ERC20 {
    ...
    constructor(
        address usd1,
        uint256 usd1decimails,
        address usd2,
        uint256 usd2decimails,
        address usd3,
        uint256 usd3decimails
    ) ERC20("WISH", "WISH") {
        isUSD[usd1] = true;
    ...
    function convert6To18(uint256 amount, uint256 decimals)
        public
        pure
        returns (uint256)
    {
    ...
    function convert18To6(uint256 usd, uint256 decimals)
        public
        pure
        returns (uint256)
    {
    ...
    function Buy(address usd, uint256 amount) external returns (bool) {
        ...
        IERC20(usd).transferFrom(msg.sender, getWishFundationAddress(), _commissionFee);
        IERC20(usd).transferFrom(msg.sender, address(this), remain);

        remain = convert6To18(remain, Decimails[usd]);
    ...
    function Sell(address usd, uint256 wishes) external returns (bool) {
        ...
        IERC20(usd).transfer(getWishFundationAddress(), _commissionFee);
        IERC20(usd).transfer(msg.sender, _remain);
        return true;
    }
}
